...
//  setPreferredSize(new Dimension(2000, 2000));
  ...
  */
 public void setGame(BufferedImage bimage, int x_parts, int y_parts, String pic)
  {
  // clear all variables
  emptyGame();
  ...
  if (bimage.getWidth() > bimage.getHeight())
	  x_parts += 1;
  else if (bimage.getWidth() < bimage.getHeight())
	  y_parts += 1;
  
  Dimension screen = Toolkit.getDefaultToolkit().getScreenSize();
  Dimension ss = new Dimension(screen.width - 200, screen.height - 100);

  puzzlePartList = new PuzzlePartList(bimage, x_parts, y_parts, ss);
  controlPanel.printPieces("" + puzzlePartList.getPartsCount());
  controlPanel.printSolved("0");
  controlPanel.startCounter();
...
//  g2.draw(repaintRect2);
 ...
 protected void stopDragging(MouseEvent e)
  ...
  if(!endMessagePrinted && puzzlePartList.getSolvedPartsCount() >= puzzlePartList.getPartsCount())
    ...
    int endMessage = JOptionPane.showInternalOptionDialog(this, "Congratulation! You solved the puzzle in " + controlPanel.getGameTime() + ".", "Congratulations",JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null, null, null);
    endMessagePrinted = true;
    
    if(endMessage == JOptionPane.OK_OPTION)
    {
        System.out.println("User clicked to end the game");
        controlPanel.sendEndNotification(controlPanel.gameTime , pic , puzzlePartList.getPartsCount());
    }
    }
  }
